...
# Define the function
def island(x, y):
    return 2 * (x ** 2) + 0.375 * (x ** 3) - x ** 4 - 4 * (y ** 2)
...
def add_perlin_noise(z, scale=3, octaves=16, persistence=2.0, lacunarity=2.0, noise_amplitude = 0.5):
    shape = z.shape
...
x = np.linspace(-1.3, 1.8, 50)
z = np.linspace(-0.8, 0.8, 50)
x, z = np.meshgrid(x, z)
